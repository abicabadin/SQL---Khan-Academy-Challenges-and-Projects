...
-- return the minimum, maximum, and average surface gravity of the objects in the solar system.
SELECT MIN(surface_gravity), MAX(surface_gravity), AVG(surface_gravity) FROM solar_system_objects;

-- return all the satellites and their masses in the solar system 
SELECT body, mass, type_of_object from solar_system_objects where type_of_object LIKE '%satellite%';

-- return all the satellites where mass is greater than the moon.
SELECT body, mass, type_of_object FROM solar_system_objects
WHERE mass >= (SELECT mass FROM solar_system_objects WHERE body = 'Moon') AND type_of_object like '%satellite%';

-- return the number of moons of Jupiter
SELECT count(body) AS Jupiter_moons FROM solar_system_objects WHERE type_of_object = 'satellite of Jupiter';

-- return the number of moons of Saturn and the average mass of the moons
SELECT count(body) AS Saturn_moons, AVG(mass) FROM solar_system_objects WHERE type_of_object = 'satellite of Saturn';
 
-- return all the planets showing the ascending order of their masses
SELECT body, mass from solar_system_objects where type_of_object LIKE '%planet (%' ORDER BY mass ASC;

-- return the satellites of the planets of the solar system with an average mass less than the mass of Earth's moon OR greater than the mean radius of the Earth's moon.
SELECT type_of_object, AVG(mass), AVG(mean_radius) from solar_system_objects where type_of_object LIKE '%satellite%' GROUP BY type_of_object HAVING AVG(mass) < (SELECT mass FROM solar_system_objects WHERE body = 'Moon') OR AVG(mean_radius) > (SELECT mean_radius FROM solar_system_objects WHERE body = 'Moon');

-- using CASE with the body and the mean_radius_rel, outputting 'Bigger than earth' for mean_radius_rel > 1, 'Earth for mean_radius_rel = 1, and 'smaller than earth for mean_radius_rel <1.
SELECT body, mean_radius_rel,
    CASE 
    ...
    END AS "Size relative to Earth"
FROM solar_system_objects;

-- use COUNT with GROUP BY to show the number of objects with each of those categories.
SELECT count(*),
 CASE 
        WHEN mean_radius_rel > 1 THEN "bigger than earth"
        WHEN mean_radius_rel = 1 THEN "EARTH"
    ...
    END AS "Size relative to Earth"
FROM solar_system_objects
GROUP by "Size relative to Earth";

-- using CASE with the body and the mass_rel, outputting 'heavier than earth' for mass_rel > 1, 'Earth for mass_rel = 1, and 'lighter than earth for mass_rel <1.

SELECT body, mass_rel,
    CASE
        WHEN mass_rel > 1 THEN "heavier than earth"
        WHEN mass_rel = 1 THEN "EARTH"
        WHEN mass_rel < 1 THEN "lighter than earth"
    END AS "Mass relative to Earth"
FROM solar_system_objects;
...
CASE
    ...
    END AS "Mass relative to Earth"
FROM solar_system_objects
GROUP BY "Mass relative to Earth";
